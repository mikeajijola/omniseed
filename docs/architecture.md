...
Company Work is runtime state, never desired state. A conversation identifies an Agent-owned dialogue and holds an opaque runtime association: explicit interaction protocol ID, runtime session ID, cursor, and server-only continuation material. A `CompanyWorkRun` is one separately auditable operational segment within that conversation. Later segments may reuse the conversation's runtime association without combining their intent, operation, proposal, plan, evidence, or outcome records. Legacy Eve-shaped session records migrate to the `eve.session/1` compatibility protocol; Eve is not the canonical Engine contract.

Work segments durably represent waits for user input, exact company approval, checks, merge, desired-revision advance, apply, and observation. When the referenced governance fact occurs, Engine writes a content-addressed continuation event. Runtime adapters claim these events with expiring leases and acknowledge completion idempotently, so restart and redelivery do not duplicate a continuation. Cancellation and terminal state are checked again at claim and completion. Changed heads and stale reviewed material are delivered as denial facts and never imply permission to apply.

Company Work is persisted through a separate optimistic store from reviewed reconciliation state, while Git remains the sole approved desired-state authority. Public projections include protocol/session identity and cursor for routing but deliberately omit continuation material, credentials, model history, prompts, tool-internal data, and hidden reasoning.
