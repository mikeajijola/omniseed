...
export const OPERATION_REGISTRY=Object.freeze([
  ...
  operation('inspect_capability','Inspect one capability, its state, realisations and evidence.','getCapability',{permissions:['read_company'],inputs:{id:'capability_id'},interfaces:['ui','lily','api','cli','controller']}),
  ...
  operation('generate_plan','Generate a proposed company change plan.','generatePlan',{permissions:['plan_company'],risk:'low',interfaces:['ui','lily','api','cli','controller']}),
  ...
  operation('apply_plan','Apply approved plan changes.','applyPlan',{permissions:['apply_plan'],mutation:true,approvalRequired:true,risk:'high',inputs:{planId:'string',approvedChangeIds:'string[]',authorization:'authorization'},interfaces:['ui','lily','api','cli','controller']}),
...
export class DeterministicIntentResolver {
  async resolve(utterance,companyContext={},availableCapabilities=OPERATION_REGISTRY) {
    ...
    if(/how would|plan|what should we do|sort out|improve|fix/.test(normalized))return result('resolved',{intent:'improve_capability',target:target?.id,candidateOperations:allowed(['inspect_capability','generate_plan','present_plan'],available)});
    ...
    if(/show me|why|capabilit/.test(normalized))return result('resolved',{intent:'inspect_capability',target:target?.id,candidateOperations:allowed(['inspect_capability','list_capabilities','present_capability'],available)});
