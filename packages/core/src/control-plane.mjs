...
export class DeterministicIntentResolver {
  async resolve(utterance,companyContext={},availableCapabilities=OPERATION_REGISTRY,interactionContext={}) {
    ...
    if(/go ahead|apply(?: the| this)? plan|do it|execute|redeploy/.test(normalized))return result('resolved',{intent:'request_execution',target:target?.id,candidateOperations:allowed(['inspect_plan','apply_plan'],available),requiresApproval:true});
