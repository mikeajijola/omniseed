...
import { attestStewardshipApproval, compileStewardshipProfile, definitionHash, MemoryStateStore, OmniSeed, ProviderRegistry } from "../src/index.js";
...
test("mandatory autonomous gates cannot be disabled", () => {
  for (const gate of ["validation", "independentReview", "unchangedHead", "successfulChecks"]) assert.throws(() => compileStewardshipProfile(declaration({ ...autonomy, gates: { ...autonomy.gates, [gate]: false } })), error => error.code === "stewardship_policy_unsafe" && error.details.disabledGates.includes(gate));
});
...
test("approval, evaluation and completion retries are idempotent with exact-once release", async () => {
  ...
  const completion = await invoke(engine, f.company, "complete_stewardship_proposal", "stewardship.propose", { proposalId: f.proposal.id, outcome: "completed", evidence: ["e1"] }, steward);
  assert.deepEqual(await invoke(engine, f.company, "complete_stewardship_proposal", "stewardship.propose", { proposalId: f.proposal.id, outcome: "failed" }, steward), completion);
