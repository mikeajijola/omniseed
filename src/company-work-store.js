...
import { migrateCompanyWorkState } from "./company-work.js";

export function emptyCompanyWorkState(companyId = null) { return { version: 0, companyId, runs: [], conversations: [], continuationEvents: [] }; }
...
export class JsonCompanyWorkStore {
  ...
  async load(companyId) {
    try {
      ...
      return migrateCompanyWorkState(state, companyId);
    } catch (error) {
...
export class MemoryCompanyWorkStore {
  ...
  async load(companyId) { return migrateCompanyWorkState(this.state.companyId ? structuredClone(this.state) : { ...structuredClone(this.state), companyId }, companyId); }
  async save(state, expectedVersion) {
