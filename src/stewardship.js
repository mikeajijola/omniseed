...
export function compileStewardshipProfile(declaration, runtime = {}, now = new Date()) {
  ...
  return {
    ...
    usage: structuredClone(runtime.stewardshipUsage ?? { active: 0, dailyChanges: 0, actions: 0, repairRounds: 0, day: now.toISOString().slice(0, 10) })
  };
}
...
export function evaluateStewardshipProposal(profile, proposal, { actorId, approval, checks = [], now = new Date() } = {}) {
  ...
  if (profile.limits.actions && usage.actions + (proposal.actionCount ?? 1) > profile.limits.actions) return stewardshipReason("stewardship_action_limit_exhausted", "The action limit is exhausted.");
